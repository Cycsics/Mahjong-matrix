...
class MentalColumn(models.Model):
    """
    文章栏目的 Model
    """
    # 栏目标题
...
# 建立博客文章类 class Mental，处理与文章有关的数据，它包含需要的字段和保存数据的行为
class MentalPost(models.Model):
    """
    文章的 Model
    """
    ...
    # 文章标题图
    avatar = models.ImageField(upload_to='mental/%Y%m%d/', blank=True)
    ...
    def get_absolute_url(self):
        return reverse('mental:mental_detail', args=[self.id])

    # 保存时处理图片
    def save(self, *args, **kwargs):
        # 调用原有的 save() 的功能
        mental = super(MentalPost, self).save(*args, **kwargs)
